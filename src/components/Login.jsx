...
import {Button, Card, CardText, Dialog, DialogBody, DialogFooter, Icon, Textfield} from "react-mdc-web";
import {handleUserLogin} from "../actions/user";
import {register, tryItOutWarning, unauthorized} from "../app.messages";
...
import {getDataWolfUsers, checkAuthentication} from "../public/utils";
	...
	componentDidMount(): void {
		keycloak.init({onLoad: "login-required"}).success(
			function (authenticated) {
				...
				let temp = keycloak.idTokenParsed;
				let email = temp.email;
				getDataWolfUsers(keycloak.token);
				...
				>
					...
					<DialogContent>
						<DialogContentText id="alert-dialog-description">
							{tryItOutWarning.map((paragraph, index) => <p key={index}
																		  className="secondary-color">{paragraph} <br/>
							</p>)}
						</DialogContentText>
					</DialogContent>
					<DialogActions>
						...
						<CardText>
							...
							<div style={{textAlign: "center"}}>
								...
								<form onSubmit={this.handleLogin}>
									...
									<Button type="submit" raised disabled={!this.validateLoginForm()}
